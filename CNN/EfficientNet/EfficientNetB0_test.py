...
class_names = [image_path.parent.name for image_path in Path(test_root).rglob("*.jpg")] # 測試圖片的類別名稱 (答案)

# 設定模型
model = tf.keras.models.load_model('EfficientNetB0_model.h5')    # 載入模型
...
class_no = sorted(list(set(class_names)))   # 類別順序
prediction_class_names = [class_no[i] for i in prediction_class_names]  # 數字轉為類別名稱 (字串)

# 計算準確度
accuracy = sum([i == j for i, j in zip(class_names, prediction_class_names)]) / len(class_names)

# 寫檔紀錄
image_names = [Path(image_path).name for image_path in image_paths] # 圖片的原始檔名
rows = zip(image_names, class_names, prediction_class_names)    # 資料
with open('EfficientNetB0_result.csv', mode='w', newline='') as file:
